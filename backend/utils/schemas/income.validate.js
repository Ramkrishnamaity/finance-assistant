...
const incomeValidation = {
  /**
   * Validation for creating income
   */
  createIncome: celebrate({
  ...
  /**
   * Validation for updating income
   */
  updateIncome: celebrate({
    [Segments.BODY]: Joi.object().keys({
   ...
   */
  incomeIdParam: celebrate({
    [Segments.PARAMS]: Joi.object().keys({
      id: Joi.string().required().hex().length(24)
        .messages({
          'string.empty': 'Income ID is required',
          'string.hex': 'Invalid income ID format',
          'string.length': 'Invalid income ID length'
        })
    })
  })
};
